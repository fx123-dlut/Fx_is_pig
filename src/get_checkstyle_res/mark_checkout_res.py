        ...
        for fix_line in git_res:
            if fix_line[fix_version_col] == this_version:
                for cs_line in cs_res[1:]:
                    if cs_line[cs_res[0].index('file')].replace('\\', '/').find(fix_line[fix_file_col]) > 0 \
                            and fix_line[fix_code_col].strip() == cs_line[cs_res[0].index('code')].strip():
                        has_used.append(git_res.index(fix_line))
                        if cs_line[-1] != 'true':
                            cs_res[cs_res.index(cs_line)] = cs_line + ['true']
                            print([fix_line[fix_version_col], this_version] + [cs_line[4]] + [cs_line[cs_res[0].index('code')]])
                        break
